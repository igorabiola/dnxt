...
-- @param port  input port
-- @param message data
-- @param rxBytes number of bytes to return via LsRead
-- @param response (boolean) defines if a reponse status is required
-- @return if response=true return true if status is ok or return nil and error message

function nxt:LsWrite( port , message, rxBytes , response )
    port = input_port[port]
    local msg = lnxtcore.LsWrite( port , message, rxBytes , response )
    local result = self:exec_cmd( msg , response )
    ...
    return  #message ,string.byte( msg, 1 , #msg )
end
